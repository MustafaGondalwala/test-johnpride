...
<div class="row">
    ...
    $default_fabric = old('default_fabric', $default_fabric);
                     ...
                     <div class="col-sm-12 col-md-6">
                        <div class="form-group{{ $errors->has('p1_cat') ? ' has-error' : '' }}">
                            ...
                            <select name="p1_cat" id="p1_cat" class="form-control" >
                                ...
                                foreach($parent_design_category as $pc) 
                                { 
                                    $selected = '';
                                        if((!empty($p1_cat_ids_arr)  && in_array($pc->id,$p1_cat_ids_arr)) || old('p1_cat') == $pc->id){
                                            $selected = 'selected';
                                        }
                                    ?>
  ...
  /* $("#p1_cat").change(function()
   {
       ...
       var p_cat_id= $("#p1_cat").val(); 
        ...
        {
                  $.ajax({
                  url: "{{url('admin/ajaxhit/getCatgory')}}", 
                  type: 'post',
                  cache: false,
                  dataType: 'html',
...
$(document).on('change', '#p1_cat', function() {
    change_p1_cat();
    //change_p2_cat();

    });
$(document).on('change', '#p2_cat', function() {
    change_p2_cat();
    });

function change_p1_cat()
{
    ...
    var token= '{{ csrf_token() }}';
       var p_cat_id= $("#p1_cat").val(); 
        ...
        {
            var pls_sel = '<option value="">Please Select</option>';
            $('#p2_cat').html(pls_sel);
            $('#category_id').html(pls_sel);
        }

}


function change_p2_cat()
{
    var token= '{{ csrf_token() }}';
       var p_cat_id= $("#p2_cat").val(); 
        if(p_cat_id)
