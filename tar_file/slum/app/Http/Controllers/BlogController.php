...
class BlogController extends Controller {
    ...
    public function index(Request $request){
        ...
        $cat_slug = (isset($request->cat))?$request->cat:'';
        ...
        $BlogCategories = BlogCategory::where(['status'=>1, 'featured'=>1])->orderBy('name')->limit(6)->get();
    ...
    public function details(Request $request){
        ...
        if(!empty($slug)){
            ...
            if(isset($blog->slug) && $blog->slug == $slug){
                ...
                $recent_blogs = Blog::where('id', '!=', $blog->id)->where(['status'=>1])->orderBy('created_at', 'desc')->limit(3)->get();
