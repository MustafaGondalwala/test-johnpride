...
// echo 'here'; die;
    ...
    public static function GetStatusHTML($status, $tbl_id=0, $class='', $id=''){
        ...
        if(is_numeric($status) && strlen($status) > 0){
            ...
            $status_str = '<a href="javascript:void(0)" class="label '.$a_label.' '.$class.'" id="'.$id.'" data-id="'.$tbl_id.'" data-status="'.$status.'" >'.$status_name.'</a>';
        }
    ...
    public static function CheckAndFormatDate($date, $to_format='Y-m-d H:i:s', $from_format=''){
        $new_date = $date;
    ...
    //echo $date; die;
        ...
        $new_date = self::DateFormat($date, $to_format, $from_format='y-m-d');

        return $new_date;
    }

    public static function DateFormat($date, $to_format='Y-m-d H:i:s', $from_format=''){
        ...
        if($date != '0000-00-00 00:00:00' && $date != '0000-00-00' && $date != ''){
            if(empty($from_format) || $from_format == ''){
                $new_date = date($to_format, strtotime($date));         
            }
            elseif($from_format == 'd-m-y' || $from_format == 'd-m-y'){
                $date_arr = explode('-', $date);
                $date_str = $date_arr[2].'-'.$date_arr[1].'-'.$date_arr[0];
                $new_date = date($to_format, strtotime($date_str));
            }
            elseif($from_format == 'd/m/Y' || $from_format == 'd/m/y'){
            ...
            elseif($from_format == 'd/m/Y H:i:s' || $from_format == 'd/m/y H:i:s'){
                $datetime_arr = explode(' ', $date);
                ...
                $new_date = date($to_format, strtotime($date_str.' '.$time));
            }
            elseif($from_format == 'd/m/Y H:i A' || $from_format == 'd/m/y H:i A'){
                $datetime_arr = explode(' ', $date);
    ...
    public static function formatAuthUserAddress($data){
        ...
        if(!empty($user) && count($user) > 0){
            ...
            $userState = ($user->userState)?$user->userState:'';
            $userCity = ($user->userCity)?$user->userCity:'';

            if(!empty($userState) && count($userState) > 0){
                if(!empty($userState->name)){
                    $addressArr[] = $userState->name;
                }
            }

            if(!empty($userCity) && count($userCity) > 0){
                if(!empty($userCity->name)){
                    $addressArr[] = $userCity->name;
                }
            }
    ...
    public static function CategoryBreadcrumbFrontend($category, $first_uri, $first_uri_name, $is_last_link = false){
        ...
        if(!empty($category) && count($category) > 0){
            ...
            if(!empty($hierarchy_arr_rev) && count($hierarchy_arr_rev) > 0){
                foreach($hierarchy_arr_rev as $cat){
                    ...
                    if(isset($cat->name)){
                        ...
                        if(is_numeric($cat->parent_id) && $cat->parent_id > 0){
                            $pCatUrl = route('products.list', ['cat[]'=>$cat->slug]);
                        }

                        $breadcrumb .= '<a href="'.$pCatUrl.'" class="btn-link" >'.$cat->name.'</a>';
                        $breadcrumb .= '&nbsp;<i aria-hidden="true" class="fa fa-angle-double-right"></i>&nbsp;';
                    }
                }
                //$breadcrumb .= '&nbsp;<i aria-hidden="true" class="fa fa-angle-double-right"></i>&nbsp;';
            ...
            if($is_last_link){
                ...
                $catUrl = route('products.list', ['cat[]'=>$category->slug]);

                if(is_numeric($category->parent_id) && $category->parent_id > 0){
                    $catUrl = route('products.list', ['pcat'=>$category->slug]);
                }
    ...
    public static function sendMail($view_path, $view_data, $to, $from, $replyTo, $subject, $params=array()){
    ...
    public static function wordsLimit($str, $limit = 150){
        if(strlen($str) <= $limit){
            return $str;
        }
        else{
