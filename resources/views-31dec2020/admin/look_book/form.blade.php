@component('admin.layouts.main')
    ...
    $id = (isset($lookBook->id))?$lookBook->id:'';
    $title = (isset($lookBook->title))?$lookBook->title:'';  
    $product_sku = (isset($lookBook->product_sku))?$lookBook->product_sku:'';  
    $url = (isset($lookBook->url))?$lookBook->url:'';
    $image = (isset($lookBook->image))?$lookBook->image:'';
    $sort_order = (isset($lookBook->sort_order))?$lookBook->sort_order:'';
    $featured = (isset($lookBook->featured))?$lookBook->featured:'';
    $status = (isset($lookBook->status))?$lookBook->status:1;
...
@slot('bottomBlock')
    ...
    $(".del_image").click(function(){
        ...
        if(conf){
            ...
            $.ajax({
                url: "{{ route('admin.look-book.ajax_delete_image') }}",
                type: "POST",
