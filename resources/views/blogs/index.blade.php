...
if(!empty($images) && count($images) > 0){
	if(!empty($images[0]->name) && $storage->exists($path.$images[0]->name)){

	?> 	
<div class="fullwidth banner_blog">
	<img src="{{url('storage/banners/'.$images[0]->name)}}" alt="{{$banner->title}}" />
	<!-- <img src="{{url('')}}/images/categorybanner.jpg" alt="John Pride" /> -->
</div>
					...
					if(!empty($blog_images) && count($blog_images)){
						foreach($blog_images as $bimg){
							if(!empty($bimg->image) && $storage->exists('blogs/thumb/'.$bimg->image)){
								...
								$backgroundImage = url('storage/blogs/thumb/'.$bimg->image);
								?>
			...
			<li>
				<a href="#" class="blog_box">
					<div class="images" style="background-image: url({{url('public')}}/images/blog-img3.jpg);">
						<img src="{{url('public')}}/images/blog-blank.png" alt="John Pride" /> 
						<div class="content">
