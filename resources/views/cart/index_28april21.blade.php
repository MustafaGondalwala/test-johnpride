	...
	<section class="fullwidth innerpage"> 
		...
		<div class="container">
			...
			<div class="sectionleft">
				...
				<div  style="display: none;"  class="offersec">
				...
				<div style="display: none;" class="freedelivery">
				...
				<ul class="cartlist">
					...
					if(!empty($cartContent) && $cartContent->count() > 0){
						...
						foreach($cartContent as $cart){
							...
							if(!empty($productInventorySize) && count($productInventorySize) > 0){
								...
								foreach($productInventorySizeArr as $inventorySize){
									...
									if($inventorySize->pivot->stock > 0){
										...
										if($inventorySize->id == $cart->size_id && $inventorySize->pivot->stock >= $cart->qty){
											$sizeStock = 1;
										}
									}
								}
							...
							$sizeIdArr[$product_id]['sizeId'] = $sizeId;
							...
							if(!empty($defaultImage) && count($defaultImage) > 0){
								...
								if(!empty($defaultImage->image) ){
									...
									$imgUrl = $defaultImage->image; //str_replace("https://www.dropbox.com/", "https://www.dl.dropboxusercontent.com/", $defaultImage->image);
							...
							<li class="listItem">
								...
								<a href="{{url('products/details/'.$product->slug)}}" class="cartimg">
									...
									if(!empty($imgUrl)){
										...
										<img src="{{$image_path.$imgUrl}}" alt="{{$product->name}}">
								...
								<div class="procont">
									...
									<div class="sizeqty">
										...
										<?php
										if($isCartItemOutOfStock == 1){
											?>
											<cite>Out of stock</cite>
										<?php
										}
										?>
						...
						//echo $productsSizesJson;
						/*pr($sizeIdArr);
						pr($productsSizesArr);
						$inStock = true;
						foreach ($productsSizesArr as $pSizeArr){
			...
			if($authCheck){
				...
				<?php*/
				if(isset($isCartItemOutOfStock) && !empty($isCartItemOutOfStock)){
					$cartProcessButton = '<a href="javascript:void(0)" class="out-of-stock">Place Order</a>';
				}
				else{
...
<script type="text/javascript" src="{{url('/')}}/js/owl.carousel.min.js"></script> 
...

		{
			...
			$(".out-of-stock").on("click", function(){
				alert('One of product is out of stock');
			});
