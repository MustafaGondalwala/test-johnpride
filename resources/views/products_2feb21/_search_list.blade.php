...
if(!empty($Brands) && count($Brands) > 0){
	...
	<div class="list-group">
		...
		foreach($Brands as $brand){
			?>
			<a href="javascript:void(0)" data-field="brand[]" data-val="{{$brand->slug}}" class="sr_list_item12">{{$brand->name}}</a>
			<?php
		}
		?>
	</div>
...
if(!empty($Products) && count($Products) > 0){
	...
	<div class="">
		...
		foreach($Products as $product){
			?>
			<a href="javascript:void(0)" data-field="keyword" data-val="{{$product->name}}" class="list-group-item sr_list_item12">{{$product->name}}</a>
			<?php
		}
		?>
	</div>
