...
<?php 
$websiteSettingsNamesArr = ['TOP_MENU_IMAGE'];
				...
				foreach ($parentCategories as $category) {
					...
					<li>
						<a href="<?php echo url('products/?pcat='.$category->slug); ?>"> {{$category->name}} </a> 
							...
							$childChildCategories = $childChildCategories->where('status', 1);
					...
					<li class="desktopshow">
						<?php if(!empty($TOP_MENU_IMAGE)){ ?><img src="{{$TOP_MENU_IMAGE}}" alt="JohnPride" border="0" /> <?php } ?>
					</li>
				</ul>
