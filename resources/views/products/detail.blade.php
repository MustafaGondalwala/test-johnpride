...
<section class="fullwidth innerlist">
	...
	<div class="single_product_page">
		...
		<?php $productImages = $product->productImages;
		...
		foreach ($productImages as $image){ ?>
			...
			<div>
				...
				<a href="{{$image_path.$imageUrl}}" data-imagelightbox="g">

					<img src="{{$image_path.$imageUrl}}" alt="{{$image->image}}" />
		...
		<div class="dtright">
						...
						<label>
							...
							<?php if($inventorySize->pivot->stock > 0){ ?>
								...
								if($authCheck){ ?>
									...
									<?php } } ?>
									<div class="sizehover">Please refer the size chart before selecting the size</div>
			...
			*/
			...
			<div class="prodt">
				...
				<div class="prod-detail"><strong>Product Details</strong> <i class="detailicon"></i></div>
    ...
    <div id="collapseOne" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
      ...
      <ul class="other_information_list">
		...
		<div class="fullwidth guaranteesec">
			...
			<div class="guarntycont">JOHNPRIDE exists for the love of fashion. We believe in empowering individuality. Our mission is to be the global platform for Plus Size Menswear Fashion, connecting creators, curators, and consumers.</div>
		...
		<div class="fullwidth fullimg" style="text-align: center;">
			...
			if(isset($detail_image) && $detail_image!=''){ ?>
				...
				<img src="{{$image_path.$detail_image}}" alt="Product Image" />
			...
			<?php */ ?> 
				...
				//prd($similarProducts);
					...
					foreach($similarProducts as $sp){
						...
						$smainImageUrl = $spImg; //str_replace("https://www.dropbox.com/", "https://dl.dropboxusercontent.com/", $spImg);
							...
							<a href="{{route('products.details', [$sp->slug])}}" class="product">
								...
								<div class="productimg">
									...
									if(!empty($spImg)){
										...
										<div class="pimg"> <img src="{{$image_path.$smainImageUrl}}" alt="{{$sp->name}}" />  </div>
			...
			<div class="similarslider owl-carousel">
				...
				<?php
					...
					foreach($recentProducts as $rp){
						...
						$rmainImageUrl = $rpImg; //str_replace("https://www.dropbox.com/", "https://dl.dropboxusercontent.com/", $rpImg);
							...
							<a href="{{route('products.details', [$rp->slug])}}" class="product">
								...
								<div class="productimg">
									 ...
									 <img src="{{url('/')}}/images/blank.png" alt="{{$product->name}}"/>
										...
										<div class="pimg"><img src="{{$image_path.$rmainImageUrl}}" alt="{{$rp->name}}" /></div>  
								...
								<div class="procont">
									...
									<div class="heading3">{{$rp->name}}</p></div>
									...
									<p>
										...
										<?php if($rp->sale_price > 0 && $rp->price > $rp->sale_price && $discount > 0 ){ ?>
...
<?php 
	if($authCheck)
	{
		...
		<a style="display: none;" href="javascript:void(0)" class="open_slide">Place Order</a>
		<?php
	}
...
$(document).on("click", ".addToCart", function(){
	...
	//fbq('track', 'Add To Cart', {currency:"INR", value:<?php //echo $productPrice; ?>});



	fbq('trackCustom', 'AddToCart', {currency:"INR", value:<?php echo $productPrice; ?>});
...
$(".prod-detail").next().addClass("addReadMore showlesscontent");
$(".prod-detail").next().next('p').addClass("next-read-more");
$(".prod-detail").next().next('p').hide();
$(".next-read-more").after( "<span class='next-more'>Read more</span>" );

$(".next-more").click(function(){
    $(".next-read-more").slideToggle("slow");
    $(this)
