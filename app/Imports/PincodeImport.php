            ...
            foreach ($rows as $row) {
               ...
               // $status = ($status_name == 'Active')?1:0;
                ...
                if(!empty($state_name))
                {
                    ...
                    $gst_code = (isset($state->gst_code))?$state->gst_code:'';
                }
                ...
                if(!empty($city_name))
                {
                   ...
                   // $city_id = (isset($city->id))?$city->id:0;
                    ...
                    else
                    {
                        ...
                        if($state_id != 0)
                        {
                            $city_arr['name'] = strtoupper($city_name);
                            $city_arr['state_id'] = $state_id;
                            $city_arr['state'] = strtoupper($state_name);
                            $city_arr['status'] = $status;
                            $city_arr['gst_code'] = $gst_code;

                           $city_id = DB::table('cities')->insertGetId($city_arr);
                        }
                    }
