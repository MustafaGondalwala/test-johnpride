...
class BrandController extends Controller
{
    ...
    public function add(Request $request){
        ...
        //prd($request->toArray()); die;         
            ...
            if(!empty($brand) && count($brand) > 0){
                ...
                $id = $isSaved->id;
                $msg="The Collections has been added successfully.";
            }
            ...
            if ($isSaved) {
                ...
                if($request->hasFile('image')) {
                    ...
                    if(!$image_result['success']){
                        //$msg="The Collections has been added successfully. But Image Can not be Added.";
                ...
                if($request->hasFile('icon')) {
                    ...
                    if(!$image_result['success']){
                        //$msg="The Collections has been added successfully. But Ican Can not be Added.";
                ...
                return back()->with('alert-danger', 'The Collections cannot be added, please try again or contact the administrator.');
            }
        }
        $data = [];
    ...
    public function ajax_delete_image(Request $request){
        ...
        if (is_numeric($image_id) && $image_id > 0 && $type =='image') {
            ...
            {
                $result['success'] = true;
                $result['msg'] = '<div class="alert alert-success alert-dismissable"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Collections image has been delete successfully.</div>';
            }
        }
            ...
            {
                $result['success'] = true;
                $result['msg'] = '<div class="alert alert-success alert-dismissable"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Collections Icon has been delete successfully.</div>';
            }
    ...
    public function delete(Request $request)
    {
        ...
        if($method=="POST"){
            if(is_numeric($id) && $id > 0)
            {
                ...
                if(!empty($brand) && count($brand) > 0){
                    ...
                    if($countProducts > 0)
                    {
                        return back()->with('alert-danger', 'This Collections cannot be removed because there are currently ' .$countProducts. ' products associated with it. Please remove the products first.');
                    }
        ...
        if($is_deleted){
            return redirect(url('admin/brands'))->with('alert-success', 'The Collections has been deleted successfully.');
        }
        else
        {
            return redirect(url('admin.brands'))->with('alert-danger', 'The Collections cannot be deleted, please try again or contact the administrator.');
        }
...
private function exportXls($query){
        ...
        $fileName = 'collection_'.date('Y-m-d-H-i-s').'.xlsx';

        return Excel::download(new BrandsExport($exportArr, $fieldNames), $fileName);
    }
