...
class CategoryController extends Controller{
    ...
    public function index(Request $request){
        ...
        $categories = $query->where(['parent_id'=>$parent_id])->paginate($limit);
    ...
    public function add(Request $request){
       //prd($request->toArray());
        ...
        if($request->method() == 'POST' || $request->method() == 'post'){
            ...
            if(empty($back_url)){
                $back_url = 'admin/categories?type';
            }
            ...
            $mob_banner_image = (isset($request->mob_banner_image))?$request->mob_banner_image:'';
            ...
            $req_data['banner_image'] = $banner_image;
            $req_data['mob_banner_image'] = $mob_banner_image;
            $req_data['slug'] = $slug;
    ...
    public function delete($category_id){
        ...
        if ($category->countProducts() && $category->countProducts()->count() > 0) {
            return back()->with('alert-danger', 'This category cannot be removed because there are currently ' . $category->countProducts()->count() . ' products associated with it. Please remove the products first.');
        }
    ...
    private function exportXls($query){
        ...
        if(!empty($categories) && $categories->count() > 0){
            foreach($categories as $category){
                ...
                $sizeArr = [];
                $sizeArr['id'] = $category->id;
                $sizeArr['name'] = $category->name;
        ...
        $fileName = 'categories_'.date('Y-m-d-H-i-s').'.xlsx';

        return Excel::download(new CategoriesExport($exportArr, $fieldNames), $fileName);
    }
