...
use App\Exports\SizeChartsExport;
...
class SizeChartController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $sizeCharts = $query->paginate($limit);
    ...
    private function exportXls($query){
        ...
        $fieldNames = ['id','title','no_of_products','status'];

        $sizecharts = $query->get();

        $exportArr = [];

        if(!empty($sizecharts) && $sizecharts->count() > 0){
            foreach($sizecharts as $sizechart){
                //prd($color->toArray());
                $no_of_products = $sizechart->countProducts();
               
                $sizeChartArr = [];
                $sizeChartArr['id'] = $sizechart->id;
                $sizeChartArr['title'] = $sizechart->title;
                $sizeChartArr['no_of_products'] = (!empty($no_of_products))?$no_of_products:0;
                $sizeChartArr['status'] = CustomHelper::getStatusStr($sizechart->status);
                

                $exportArr[] = $sizeChartArr;
            }
        }
        ...
        $fileName = 'size_charts_'.date('Y-m-d-H-i-s').'.xlsx';

        return Excel::download(new SizeChartsExport($exportArr, $fieldNames), $fileName);
    }
