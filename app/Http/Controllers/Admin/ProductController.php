...
class ProductController extends Controller{
    ...
    public function index(Request $request){
        ...
        $collection_data = Brand::where(['status'=>1])->orderBy('name')->get();
        ...
        if(!empty($collection_id)){
            $productQuery->where('collection_id', $collection_id);
        }
    ...
    private function exportXls($productQuery){
        ...
        $select = ['id','style_id','sku','folder_path','desktop_image','mobile_image','name','slug','collection_id','manufacturer','country_origin','hsn','business_unit','description','gender','video','color_id','size_chart_id','price','sale_price','gst','weight','delivery_duration','sort_order','stamp','featured','trending','net_qty','product_type','standard_size',
        'age_group','brand_color','base_color','fashion_type','prod_usage','year','season','tags','across_shoulder','bust','chest','front_length','to_fit_bust','sleeve_length','to_fit_waist','waist','popularity','new_arrival','status','meta_title','meta_keyword','meta_description','created_at','updated_at'];
        ...
        if(!empty($products) && $products->count() > 0){
            foreach($products as $product){
                ...
                $otherImages = $productImages->where('is_default',0)->where('is_reverse',0);
                $main_image = '';
                if(isset($mainImage['image'])){
                $main_image = $mainImage['image'];
                }
                $reverseImage = $productImages->where('is_reverse',1)->where('is_default',0)->first();
                $reverse_image = '';
                ...
                //prd($productCollections);
                $collectionsArr = [];
                if(!empty($productCollections) && count($productCollections) > 0){
                    $collectionsArr = $productCollections->pluck('id')->toArray();
                }
                ...
                if(!empty($collectionsArr)){
                    $collectionsIds = implode(',', $collectionsArr);
                }

                $related_category = DB::table('related_product_categories')->where('product_id', $product->id)->first();

                $related_cat_id = '';

                if(!empty($related_category)){
                    $related_cat_id = isset($related_category->category_id) ? $related_category->category_id:'';
                }
                ...
                $productArr['folder_path'] = $product->folder_path;

                $productArr['name'] = $product->name;
                //$productArr['slug'] = $product->slug;
                $productArr['collection_id'] = $collectionsIds;
                $productArr['category_id'] = $category_id;
                $productArr['related_category'] = $related_cat_id;
                ...
                $featured = ($featured == 1)?'Yes':'No';

                $new_arrival = isset($product->new_arrival)?$product->new_arrival:'';
                $new_arrival = ($new_arrival == 1)?'Yes':'No';

                $trending = isset($product->trending)?$product->trending:'';
                $trending = ($trending == 1)?'Yes':'No';

                $popularity = isset($product->popularity)?$product->popularity:'';
                $popularity = ($popularity == 1)?'Yes':'No';

                $status = isset($product->status)?$product->status:'';
                $status = ($status == 1)?'Active':'Inactive';

                $productArr['best_seller'] = $featured;
                $productArr['new_arrival'] = $new_arrival;
                $productArr['trending'] = $trending;
                $productArr['popularity'] = $popularity;

                $productArr['sort_order'] = $product->sort_order;
                $productArr['status'] = $status;
                ...
                $productArr['desktop_image'] = $product->desktop_image;
                $productArr['mobile_image'] = $product->mobile_image;
                ...
                if(isset($productImages) && !empty($productImages))
                {
                    ...
                    {
                        $productArr['Image'.$i] = $value->image;
    ...
    public function add(Request $request){
        ...
        $related_product_categories = Category::where(['status'=>1])->where('parent_id', '!=' , 0)->orderBy('name')->get();
        ...
        if(!empty($product_id)){
            ...
            //pr($relatedProductCategoryAr[0]->category_id);
            ...
            $sel_related_product_cat=$relatedProductCategoryAr[0]->category_id;
                ...
                /*foreach($relatedProductCategoryAr as $ex ){
                    $sel_related_product_cat[] = $ex->category_id;
                }*/
    ...
    public function save(Request $request, $product, $product_id){
        ...
        $new_arrival = (isset($request->new_arrival))?$request->new_arrival:0;
        ...
        $req_data = $request->except(['_token', 'product_id','related_product_cat', 'p1_cat', 'p2_cat', 'category', 'images', 'image_ids', 'is_default','is_reverse', 'sizes', 'attr', 'count_images', 'back_url','collection_id']);

        $slug = CustomHelper::GetSlug('products', 'id', $product_id, $request->name);

        $price = isset($request->price) ? $request->price:'';
        $sale_price = isset($request->sale_price) ? $request->sale_price:'';

        $gst = 5;
        $priceForGst = 0;

        if(is_numeric($sale_price) && $sale_price < $price && $sale_price > 0)
        {
          $priceForGst = $sale_price;
        }
        else
        {
          $priceForGst = $price;
        ...
        // if(isset($price) && isset($sale_price) && $price > 0 && $sale_price > 0){
        //     $priceForGst = $price;
        //     if($sale_price < $price){
        //         $priceForGst = $sale_price;
        //     }
        ...
        if(empty($sale_price) && $price == '1049'){
            $gst = 5;
        }
        ...
        $req_data['new_arrival'] = $new_arrival;
        ...
        if ($isSaved) {
            ...
            $this->saveRelatedProductCategories($request, $product_id);

            if(!empty($req_product_id)){
                $this->saveImageLinks($request, $product_id);
            }
            else{
                $this->saveDefaultImageLinks($request, $product_id);
            }

            $this->saveCollections($request, $product_id);
    ...
    private function saveDefaultImageLinks($request, $product_id){

        $folder_path = isset($request->folder_path)?$request->folder_path:'';
        $sku = isset($request->sku)?$request->sku:'';
    ...
    private function saveCollections($request, $product_id){
    ...
    private function saveRelatedProductCategories($request, $product_id){

        if(is_numeric($product_id) && $product_id > 0){
            $category_data = [];            
            $category_id = (isset($request->related_product_cat))?$request->related_product_cat:0;
    ...
    public function inventory(Request $request){
        ...
        $product = ($productId!=0)?Product::find($productId):'';
        $sku = (count($product) > 0 && !empty($product))?$product->sku:'';
        ...
        if($method == 'POST' || $method == 'post'){
            ...
            if(!empty($sizeName))
            {
                $productInventory->sku = $sku.'_'.$sizeName;
            }
    ...
    public function inventoryList(Request $request){
        ...
        if(!empty($sortBy) && $sortBy == 'top_selling'){
            $inventoryQuery->whereRaw("id IN (select product_id from order_items Group By order_items.product_id Order By sum(order_items.qty) desc)");
        }

        if(!empty($name)){
            $inventoryQuery->where(function($query) use($name){
                $query->where('name', 'like', '%'.$name.'%');
                $query->orWhere('sku', 'like', '%'.$name.'%');
            });
        }
        ...
        if(!empty($sku)){
            $inventoryQuery->where('sku', 'like', '%'.$sku.'%');
        }

        if(is_numeric($category) && $category > 0){
            $inventoryQuery->whereHas('productCategories', function ($query) use ($category) {
                $query->where('category_id', $category);
            });
        }

        if(is_numeric($price) && $price > 0){
            $inventoryQuery->where('price', $price_scope, $price);
        }

        if(is_numeric($stock) && $stock > 0){
            $inventoryQuery->where('stock', $stock_scope, $stock);
        }
        ...
        if(!empty($from_date)){
            $inventoryQuery->whereRaw('DATE(created_at) >= "'.$from_date.'"');
        }

        if(!empty($to_date)){
            $inventoryQuery->whereRaw('DATE(created_at) <= "'.$to_date.'"');
        }
    ...
    private function exportInventoryList($inventoryQuery){
        ...
        if(!empty($inventories) && $inventories->count() > 0){
            foreach($inventories as $inventory){
                ...
                $productID = (!empty($product) && count($product) > 0)?$product->id:'';
                $productName = (!empty($product) && count($product) > 0)?$product->name:'';
                
                $inventoryArr['product_id'] = $productID;
                $inventoryArr['product_name'] = $productName;
